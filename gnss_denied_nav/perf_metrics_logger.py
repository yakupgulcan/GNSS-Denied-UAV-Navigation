...
class AdvancedLogger(Node):
    def __init__(self):
        ...
        # Header: Zaman, İşlem Süresi(ms), Durum, Tahmin X, Tahmin Y, Gerçek X, Gerçek Y, Hata(m)
    ...
    def metrics_cb(self, msg):
        ...
        # Gelen veri: "status,duration_ms,est_x,est_y"
        try:
            parts = msg.data.split(',')
            algo_name = parts[0] # YENİ
            status = parts[1]
            duration = float(parts[2])
            est_x = float(parts[3])
            est_y = float(parts[4])
            
            # Hata Hesapla (Sadece MATCH ise anlamlıdır)
            error = 0.0
            if "MATCH" in status:  # Hem "MATCH" hem "MATCH_INIT" durumlarını kapsar
                error = ((est_x - self.current_true_x)**2 + (est_y - self.current_true_y)**2)**0.5
            
            # Kaydet
            t = time.time()
            self.writer.writerow([t, algo_name, duration, status, est_x, est_y, self.current_true_x, self.current_true_y, error])
            self.csv_file.flush()
            
        except Exception as e:
            self.get_logger().error(f"Parse Hatası: {e}")
