#!/usr/bin/env python3
"""
v13_brisk: Fixed Clock Sync Issue + Low Latency Configuration
"""
        ...
        try:
            ...
            if self.state == "NORMAL":
                if (now - self.last_keyframe_time) > self.KEYFRAME_INTERVAL:
                    kp, des = self.localizer.brisk.detectAndCompute(cv_image, None)
            ...
            if db_match_pos is not None:
                ...
                if dist_error < acceptable_diff:
                    ...
                    self.get_logger().warn(f"Outlier: {dist_error:.1f}m")
